...
if (!notificationButton || !helpButton) return;
...
overlay.innerHTML = `
  <section class="dashboard-tools-panel" id="notificationsPanel" role="dialog" aria-modal="true" aria-labelledby="notificationsTitle" hidden>
    <div class="dashboard-tools-head">
      <div>
        ...
        <p id="notificationSummary">Checking your business status…</p>
      </div>
      <button class="icon-btn dashboard-tools-close" data-close-tools aria-label="Close notifications">×</button>
    </div>
    <div class="notification-list" id="notificationList"></div>
    <div class="dashboard-tools-foot">
      <button class="btn" id="refreshNotifications">Refresh</button>
      <button class="btn primary" id="markNotificationsRead">Mark all as read</button>
    </div>
  </section>
  ...
  <section class="dashboard-tools-panel help-panel" id="helpPanel" role="dialog" aria-modal="true" aria-labelledby="helpTitle" hidden>
    <div class="dashboard-tools-head">
      <div>
        <h2 id="helpTitle">NATRA Help</h2>
        <p>Quick guidance for the main dashboard and daily workflow.</p>
      </div>
    ...
    <div class="help-grid">
      <article class="help-card">
        <b>Dashboard</b>
        <span>Monitor sales, profit, cash flow, stock value and items needing attention.</span>
        <button class="link-btn" data-page="dashboard">Open Dashboard</button>
      </article>
      <article class="help-card">
        <b>Inventory</b>
        <span>Products, stock adjustments, low-stock items and stock movement are kept together.</span>
        <button class="link-btn" data-page="products">Open Inventory</button>
      </article>
      <article class="help-card">
        <b>Sales & POS</b>
        <span>Create a sale from New Sale, then review completed sales and returns.</span>
        <button class="link-btn" data-page="pos">Open New Sale</button>
      </article>
      <article class="help-card">
        <b>Cash Flow</b>
        <span>Review income, expenses, transfers and the closing balance.</span>
        <button class="link-btn" data-page="cashflow">Open Cash Flow</button>
      </article>
      <article class="help-card">
        <b>Settings</b>
        <span>Configure the application and persistent business preferences.</span>
        <button class="link-btn" data-page="settings">Open Settings</button>
      </article>
      <article class="help-card">
        <b>Keyboard shortcuts</b>
        <span><kbd>Ctrl + K</kbd> focuses global search · <kbd>?</kbd> opens Help · <kbd>Esc</kbd> closes this window.</span>
      </article>
    </div>
...
const readKey = "natra.dashboard.notifications.readAt";
...
function openTools(panel) {
  ...
  if (panel === "notifications") refreshNotifications();
}
...
function getNotificationItems() {
  ...
  if (lowStockCount > 0) {
    items.push({
      ...
      text: "Review products that are at or below their minimum stock level.",
      page: "lowstock"
    });
  }

  const backupText = document.getElementById("backupLabel")?.textContent?.trim() || "";
  if (backupText && /checking|never|overdue|failed|error/i.test(backupText)) {
    items.push({
      id: "backup",
      type: /failed|error/i.test(backupText) ? "danger" : "info",
      icon: "↻",
      title: "Backup status needs attention",
      text: backupText,
      page: "backup"
    });
  }
  ...
  if (!items.length) {
    items.push({ id: "all-clear", type: "success", icon: "✓", title: "All clear", text: "No active dashboard alerts were detected.", page: "dashboard" });
  }
...
function renderNotifications() {
  ...
  const unread = readAt === 0 ? items.length : 0;
  if (notifyDot) {
    notifyDot.textContent = unread > 9 ? "9+" : String(unread);
    notifyDot.hidden = unread === 0;
  }
  notificationSummary.textContent = unread ? `${unread} notification${unread === 1 ? "" : "s"} need your attention.` : "You're up to date.";
  notificationList.innerHTML = items.map((item) => `
    <button class="notification-item ${item.type}" data-page="${item.page}" type="button">
      <span class="notification-icon">${item.icon}</span>
      <span class="notification-copy"><b>${escapeHtml(item.title)}</b><small>${escapeHtml(item.text)}</small></span>
...
const observer = new MutationObserver(() => {
  if (!activePanel || activePanel === "notifications") renderNotifications();
});
